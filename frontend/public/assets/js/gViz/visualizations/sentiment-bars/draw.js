...
gViz.vis.sentimentBars.draw = function () {
  "use strict";
        ...
        case 'run':
          ...
          switch (action) {
            ...
            case 'draw':
              ...
              _var.bars = _var.g.selectAll('.bar').data(_var._data.paragraphs);
              _var.bars.exit().remove();
              _var.bars = _var.bars.enter().append('rect').attr("class", 'bar').merge(_var.bars);

              _var.bars
                .attr("x", function (d, i) { return _var.xScale(i); })
                .attr("width", _var.xScale.bandwidth())
                .attr("y", function (d) { return _var.yScale(d["participation"]); })
